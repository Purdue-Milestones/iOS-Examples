...
//  MicrovestApp
//
//  Created by Carolina Solís on 4/24/22.
//
...
class TempViewController: UIViewController {
    ...
    var receivedData = SingletonClass.shared.GeneralDataTemp //received data displayed on label and graph
    ...
    @IBOutlet weak var TempBox: LineChartView!
    ...
    @objc func TimeKeeper(){
        ...
        displayGraph(dataDisplaying: SingletonClass.shared.GeneralDataTemp)
        HeartRate(dataDisplaying: SingletonClass.shared.GeneralDataTemp)
    }
    ...
    func displayGraph(dataDisplaying: [Float]) {
        ...
        let line1 = LineChartDataSet(entries: lineChartEntry, label: "Degrees")
        ...
        TempBox.setScaleEnabled(true)
        TempBox.pinchZoomEnabled = true
        
        TempBox.data = lineData

        // Settings for the chartBox
        TempBox.chartDescription!.text = "Retrieving Data..."
    }
    ...
    func HeartRate(dataDisplaying: [Float]){
        ...
        tempLabel.text = String(format:"%.2f", avgArrayValue)
